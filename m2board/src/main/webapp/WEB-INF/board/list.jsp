...
<body>
	...
	<table width="600">
		...
		<tr>
			<td colspan="5" align="right">현재 페이지 : ${pg }, 총 페이지 : ${pageCount }</td>
		</tr>
		<tr>
			<th>글번호</th>
			<th>제목</th>
	...
	<table>
		<tr>
			<td>
			<c:if test="${startPage != 1 }"><a href="BoardList.do?pg=${startPage - 1 }">[이전]</a></c:if>
			<c:forEach var="p" begin="${startPage }" end="${endPage }">
			<c:if test="${pg == p }">${p }</c:if>
			<c:if test="${pg != p }"><a href="BoardList.do?pg=${p }">${p }</a></c:if>
				</c:forEach>
			<c:if test="${endPage != pageCount }"><a href="BoardList.do?pg=${endPage + 1 }">[다음]</a></c:if>
			</td>
		</tr>
	</table>
