...
<script>
	var user_in_count = -1;
	var user_id = "";
	//-1 : 시스템 오류, 0 : 사용할수 있는 아이디, 1 : 사용할 수 없는 아이디
	...
	function validate() {
		var f = document.regist; //<form>태그에 name으로 접근
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				if (user_id_count == 1) {
					...
					document.getElementById('check_id_result').innerHTML = '<font color="blue">' + user_id + '는 사용가능한 아이디입니다.</font>';
					return;
				} else {
					alert('시스템 오류 입니다.');
				}
		...
		<table border="1">
			...
			<tr>
				...
				<td><input type="text" name="user_id" id="user_id" onkeyup="change_user_id()" />
					<div id="check_id_result"> 영문자 + 숫자 3 ~ 12 문자로 작성하세요.</div></td>
			</tr>
			<tr>
				<th>이름</th>
				<td><input type="text" name="user_name" id="user_name" /> 2 ~ 6 문자로 작성하세요.</td>
			</tr>
			<tr>
				<th>비밀번호</th>
				<td><input type="password" name="user_pw" id="user_pw" /> 4 ~ 20 문자로 작성하세요.</td>
			</tr>
			<tr>
				<th>비밀번호 확인</th>
